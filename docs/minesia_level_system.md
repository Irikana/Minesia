...
minesia.level.level_up=恭喜！你的等级已提升到 %s 级！
...
- 奖励发放：`[Minesia] 标记玩家 玩家名 已获取 X 级奖励`
...
```javascript
if (reward.rubyCoin) {
    this.giveItem(player, "minesia:ruby_coin", reward.rubyCoin);
}
```
